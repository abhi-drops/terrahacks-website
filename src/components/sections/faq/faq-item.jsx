...
export default function FAQItem({ item, expanded, setExpanded }) {
    ...
    return (
        <li className="lg:text-sm xl:text-base w-full min-h-[50%] max-h-[75%] md:min-h-full md:max-h-full">
            ...
            <div className={`fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div ref={contentRef} className={`relative flex flex-col items-center justify-center bg-white w-11/12 md:w-1/2 h-3/4 md:h-1/2 rounded-md shadow-md transition-transform transform ${isVisible ? 'scale-100' : 'scale-95'}`}>
                    <button
